...
bool eae6320::Graphics::TextureHelper::LoadTextureFromFile(Texture& i_texture, const char* const i_texturePath, const Context& i_context)
{
	...
	// Create a new texture and make it active
	{
		...
		glGenTextures(textureCount, &i_texture.textureData);
	...
	// Verify that the file is a valid DDS
	{
		...
		else
		{
			...
			std::stringstream errorMessage;
			errorMessage << "The texture file \"" << i_texturePath << "\" isn't a valid DDS. The Four CC is \"" << fourCcString << "\""
				" instead of \"DDS \"";
			eae6320::UserOutput::Print(errorMessage.str());
			goto OnExit;
		}
	...
	GLenum format;
	{
		...
		else
		{
			...
			std::stringstream errorMessage;
			errorMessage << "The texture file \"" << i_texturePath << "\" has an unsupported format \"" << fourCcString << "\"";
			eae6320::UserOutput::Print(errorMessage.str());
			goto OnExit;
		}
	}
...
OnExit:
	...
	if (wereThereErrors && (i_texture.textureData != 0))
	{
		const GLsizei textureCount = 1;
		glDeleteTextures(textureCount, &(i_texture.textureData));
