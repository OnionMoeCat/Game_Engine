...
namespace
{
		...
		const std::vector<sMaterialInfo>& i_materialInfo )
	{
		...
		if (i_indexBuffer.size() % 3 != 0)
		{
			MGlobal::displayError(MString("Index buffer length: " + i_indexBuffer.size()) + ", times of 3 expected for triangle indexes");
			...
			{
				{
					...
					for (int i = 0; i < i_vertexBuffer.size(); i++)
					{
						fout << "\t\t" << "{\n";
						{
							...
							fout << "\t\t\t" << "texcoord = {" << i_vertexBuffer[i].u << ", " << i_vertexBuffer[i].v << "},\n";
						}
						fout << "\t\t" << "},\n";
					}
					fout << "\t" << "},\n";
